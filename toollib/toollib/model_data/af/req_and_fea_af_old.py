...
models = [
    "app_id_base_v1","app_id_comp_v1","app_id_gcate_v1",'app_id_loan_v1','app_id_cate_v1',
    'sms_id_base_v1',
    'loan_id_base_v2','loan_id_base_v3_2',
    'call_id_base0_v1',
    'cross_id_base_v1',
    'dev_id_base_v1','dev_th_base_v2',
    'ui_id_base_v2'
]
...
logging.info(f'req_dir:{req_dir}')
...
def calc_features(file,model,uri,max_workers,file_name,tqdm_disable):
...
for ser_name,model_list in feature_maps.items():
    ...
    for model in model_list:
        save_dir = os.path.join(feature_dir,model)
        ...
        for file in file_paths:
            ...
            if write_mode == 'increament':
                try:
                    ...
                    logging.info(f"{save_path}已存在,无需计算! {tmp.shape}")
                except:
                    calc_features(file,model,uri,max_workers,file_name,tqdm_disable)
